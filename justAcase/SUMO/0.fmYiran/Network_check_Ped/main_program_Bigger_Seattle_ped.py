...
OD_data = genfromtxt('ped_od.csv', delimiter=',')
row, colume = np.shape(OD_data)
with open("ped_od.txt","w") as OD:
    OD.write("$OR;\n")
...
os.system('od2trips -n Taz_bigger_Seattle_all_ped.add.xml -d Bigger_Seattle_OD.txt -o Bigger_Seattle_ped_rou.xml --pedestrians --timeline.day-in-hours \
...
traci.start([sumoBinary, "-c", "Bigger_Seattle.sumocfg", '--start'])
